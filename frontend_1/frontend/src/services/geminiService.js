// Gemini AI Service - Blood Sample Management System
// Uses Google's Gemini 2.5 Flash model for clinical reasoning
 ...
 */
async function geminiRequest(messages, systemPrompt = '', options = {}) {
  const activeKey = getApiKey();
  if (!activeKey) {
    throw new Error('GEMINI_API_KEY not configured. Please configure it in the AI Settings.');
  }
  ...
  const response = await fetch(getApiUrl(activeKey), {
    method: 'POST',
    headers: {
...
${JSON.stringify(testData, null, 2)}`;
  ...
  try {
    const content = await geminiRequest(
      ...
      systemPrompt,
      { maxTokens: 1500, temperature: 0.15, jsonMode: true }
    );
    ...
    console.error('Gemini analyzeBloodTestResults failed:', error);
    throw error;
  }
}
...
${JSON.stringify(patientData, null, 2)}`;
  ...
  try {
    const content = await geminiRequest(
      [{ role: 'user', content: userMessage }],
      systemPrompt,
      { maxTokens: 1200, temperature: 0.2, jsonMode: true }
    );
    ...
    console.error('Gemini generatePatientRiskAssessment failed:', error);
    throw error;
  }
}
...
${JSON.stringify(analyticsData, null, 2)}`;
  ...
  try {
    const content = await geminiRequest(
      [{ role: 'user', content: userMessage }],
      systemPrompt,
      { maxTokens: 1500, temperature: 0.25, jsonMode: true }
    );
    ...
    console.error('Gemini analyzePopulationTrends failed:', error);
    throw error;
  }
}
...
Do NOT provide direct diagnoses. Always note uncertainty where appropriate.`;
  ...
  return await geminiRequest(messages, systemPrompt, { maxTokens: 1000, temperature: 0.35 });
}
...
${JSON.stringify(reportData, null, 2)}`;
  ...
  try {
    const content = await geminiRequest(
      [{ role: 'user', content: userMessage }],
      systemPrompt,
      { maxTokens: 1000, temperature: 0.15, jsonMode: true }
    );
    ...
    console.error('Gemini generateReportSummary failed:', error);
    throw error;
  }
}

/**
 * Formats messy dictation / notes into structured summaries
 */
...
${JSON.stringify(samples, null, 2)}`;
  ...
  try {
    ...
    console.error('Gemini detectSampleAnomalies failed:', error);
    throw error;
  }
}
