      ...
      try {
        ...
        if (response.data && response.data.length > 0) {
          const mapped = response.data.map(r => ({
            ...
            generatedAt: new Date(r.generatedAt || r.createdAt).toLocaleDateString(),
            status: r.isApproved ? 'Completed' : 'Pending Review',
        ...
        console.error('Failed to load reports from backend, using mock:', err);
        setReports(mockReports);
      } finally {
        setLoading(false);
      }
